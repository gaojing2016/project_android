...
public class Main extends ActivityGroup {
	...
	public Main(){
		...
		mEngine = (Engine)Engine.getInstance(); //gaojing:创建Engine实例mEngine
		mEngine.setMainActivity(this); //gaojing:Sets the main activity to use as context in order to query some native resources.
    	mScreenService = ((Engine)Engine.getInstance()).getScreenService(); //gaojing:创建screenservice实例mScreenService
	}
    ...
    /* gaojing:onsaveInstanceState方法是用来保存Activity的状态的。当一个Activity在生命周期结束前，会调用该方法保存状态。
